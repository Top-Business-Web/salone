...
use SendMail;
    ...
    { 
       ...
       if($lang != NULL){
        $description = DB::table('vendor')
                     ->select($lang.'_description as description','shop_type','vendor_name','vendor_id', 'vendor_phone', 'owner','vendor_logo','vendor_loc','lat','lng')
                     ->where('vendor_id', $vendor_id)
                     ->first();
       }else{
           $description = DB::table('vendor')
                      ->select('description','shop_type','vendor_name','vendor_id', 'vendor_phone', 'owner','vendor_logo','vendor_loc','lat','lng')
                     ->where('vendor_id', $vendor_id)
                     ->first();
       }
       ...
       $type= $description->shop_type;
        ...
        if($lang){
          ...
          $data = array(
            ...
            "vendor_phone"=>$description->vendor_phone,
            "vendor_whatsapp"=>'https://api.whatsapp.com/send?phone='.$description->vendor_phone,
        ...
        if($description){
            $message = array('status'=>'1', 'message'=>'Location details', "data"=>$data);
          return $message;
        }  
        else{
    ...
    { 
       ...
       $nearbystore = DB::table('vendor')
        ...
        if($pr != NULL){ 
            $message = array('status'=>'1', 'message'=>'Locations where service is available:', 'data'=>$pr);
            return $message;
           }
           else{
                $message = array('status'=>'0', 'message'=>'No locations nearby registered to offer that service yet.');
