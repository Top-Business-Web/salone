   ...
   public function book_now(Request $request)
    {
        ...
        if($user_detail->count()<1) return array('status'=>'0', 'message'=>'User account entered incorrectly. Ask book services for more information.');

        if($user_detail->block==1) return array('status'=>'0', 'message'=>'You are blocked by admin you cannot book services.');
      ...
      foreach($lang as $langs){
        ...
        $oo = DB::table('orders')
           ...
           $ordersuccessed = DB::table('orders')
        	...
        	$message = array('status'=>'0', 'message'=>'Request failed! Try again later');
        	return $message;
        }
 ...
 public function checkout(Request $request)
    {
       ...
       if($cart->count()<1) return array('status'=>'0', 'message'=>'Cart ID not found! Unable to complete');         
       if($getD->count()<1) return array('status'=>'0', 'message'=>"Oops! That vendor doesn't exist. Unable to complete");         
...
while ($lastNumber <= $end_time) // loop between time
    ...
    for($i = 1; $i < count($array_of_time1) - 1; $i++)
    {
        ...
        if($orders > $totorders){
            ...
            $new_array_of_time1[] =array('timeslot'=>'' . $array_of_time1[$i] . ' - ' . $array_of_time1[$i + 1], 'availability'=>'true');

        }
        else{

             $new_array_of_time1[] =array('timeslot'=>'' . $array_of_time1[$i] . ' - ' . $array_of_time1[$i + 1], 'availability'=>'false');
        }
