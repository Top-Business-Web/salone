    ...
    if(Session::has('cityadmin'))
     {
        ...
        $deliveryRange = intval($request->deliveryRange);
        ...
        if(!is_int($deliveryRange)||$deliveryRange<=0){
          return redirect()->back()->withErrors('Delivery range entered requires to be an integer.');
        }
        ...
        if(!is_null($mapset)){
          if(is_null($checkmap)||is_null($checkmap->map_api_key)) return redirect()->back()->withErrors("Please check the API Key configuration for Google Maps Location Services before to continue.");
          ...
          if($mapset->mapbox == 0 && $mapset->google_map == 1){
            ...
            if(empty($response->results)) return redirect()->back()->withErrors('Address not valid. Enter a new valid address to continue.');
          ...
          if(is_null($lat)||is_null($lng)) return redirect()->back()->withErrors("We were unable to locate the address entered. Please check your API Key for Location Services before to continue.");
        } else {
          return redirect()->back()->withErrors("You should check your general settings and select a Location service from the list before you continue.");
        }
     ...
     if(Session::has('cityadmin'))
      {
       ...
       if($password!=$password2){
         ...
         if($password!="" && $password2!="")
        {
            ...
            {
                $new_pass=Hash::make($password);
                $value=array('cityadmin_id'=>$cityadmin_id,'vendor_name'=>$vendor_name,'vendor_logo'=>$vendor_image,'vendor_email'=> $vendor_email,'vendor_phone'=> $vendor_phone, 'vendor_loc'=>$address,'lat'=>$lat,'lng'=>$lng,'owner'=>$owner, 'delivery_range'=>$deliveryRange, 'updated_at'=>$updated_at,'vendor_pass'=>$new_pass,'shop_type'=>$shop_type,'admin_share'=>$admin_share,'description'=>$description);
            }
        }
        else
        {
            $value=array('cityadmin_id'=>$cityadmin_id,'vendor_name'=>$vendor_name,'vendor_logo'=>$vendor_image,'vendor_email'=> $vendor_email,'vendor_phone'=> $vendor_phone, 'vendor_pass'=>$new_pass,'vendor_loc'=>$address,'lat'=>$lat,'lng'=>$lng,'owner'=>$owner, 'delivery_range'=>$deliveryRange, 'updated_at'=>$updated_at,'shop_type'=>$shop_type,'admin_share'=>$admin_share,'description'=>$description);
        }
