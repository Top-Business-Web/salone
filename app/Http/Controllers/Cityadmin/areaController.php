    ...
    if(Session::has('cityadmin'))
     {
    	...
    	return view('cityadmin.area.Addarea',compact("cityadmin_email","cityadmin","city","map1","mapset","mapbox"));
         }
    else
         {
            return redirect()->route('cityadminlogin')->withErrors($this->login_message);
    ...
    if(Session::has('cityadmin'))
     {	
      ...
      if(!is_null($mapset)){
        ...
        if(is_null($lat)||is_null($lng)) return redirect()->back()->withErrors('Address not valid. Enter a new valid address to continue.');
      } else {
